...
import classes.MD5Sum;
import classes.Ray;
import classes.RayTracer;
...
/**
 * @author majid
 * this is a big test class which contains important test cases.
 * within this class we do some integrity tests to make sure
 * that our camera and light source class are working properly
 * during the ray tracing process. further more we test some 
...
public class RayTracerTest extends TestCase {
	...
	/**
	 * this method is written to test limitation of reflection depth.
	 * in order to test reflection depth we first create a scene with two
	...
	/**
	 * in this test case first we render a scene with 2 light sources. Then we 
	 ...
	 * brighter we pass the test. it shows that lights are correctly rendered 
	 * in our project.
	 */
	public void testWithDifferentLights(){
	...
	/**
	 * in this test case we trace a known scene. and write the result to a
	 ...
	 */
	public void testWithDifferentCameraPosition(){
		...
		testScene.camera.location = new Vector3D(30, -50, -100);
		testTracer.rayTraceScene("testResult.ppm", testScene, viewport, 5, false ,"1" ,1);
	...
	public void testShadowing(){
		...
		// our plane shape.
		info.distance = 55.172;
		info.element = testPlane;
		info.hitCount = 1;
		info.isHit = true;
		info.normal = new Vector3D(0.447, 0, -0.894);
		info.position = new Vector3D(-8.830, 5.184, 163.289);
		Vector3D rayd = new Vector3D(-0.085, 0.494, -0.864);

		// calculate shadow, create ray from intersection point to light
		Ray shadowray = new Ray(info.position, rayd);
		...
		assertFalse(shadow.isHit);

		
		// then we create an intersection point on a shadowed space of 
		// our plane shape.
		info.distance = 72.312;
		info.element = testPlane;
		info.hitCount = 1;
		info.isHit = true;
		info.normal = new Vector3D(0.447, 0, -0.894);
		info.position = new Vector3D(11.772, -16.265, 173.591);
		rayd = new Vector3D(-0.206, 0.560, -0.802);

		// calculate shadow, create ray from intersection point to light
		shadowray = new Ray(info.position, rayd);

		// find any element in between intersection point and light
		shadow = testTracer.testIntersection(shadowray, testScene);
		// this point should not be in a shadow!
		assertTrue(shadow.isHit);
